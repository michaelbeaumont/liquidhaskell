...
import Prelude hiding (filter, reverse, (++)) -- LIQUID
...
member a s = undefined -- LIQUID isJust (findTag a s)
...
-- LIQUID TODO look this again!
{-@ insertUp_ :: x:a 
              -> (Maybe (UStack a))  
              -> {v:StackSet i l a s sd | not (StackSetElt x v)}
              -> (StackSet i l a s sd)@-}
insertUp_ :: a 
          -> Maybe (Stack a) 
          -> StackSet i l a s sd 
